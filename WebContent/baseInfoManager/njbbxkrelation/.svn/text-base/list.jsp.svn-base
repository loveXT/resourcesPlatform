...
<body>
	...
	<form id="pageForm" class="sm_search" action="" method="post">
		...
		<input type="hidden" name="P_NJBBXKRELATION_KCID_S_WD" value="">
	</form>
	<div class="sm_list">
		<!--列表  标题-->
		<table>
			<tr>
				...
				<td style="width: 120px;">所属年级</td>
				<td style="width: 120px;">适用版本</td>
			</tr>
		</table>
		<!--列表  内容-->	
		<table>
			<%
				...
				PageInfo pageInfo = njbbxkRelation.findPageInfoByNjbbxkRelation(nowPage, pageSize, "_", request);
			...
			<tr>
				...
				<td style="width: 30px;" class="sm_index">
					<input type="checkbox" value="<%=Validate.isNullToDefault(map.get("KCID"),"")%>">
				</td>
				<td style="width: 120px;"><%=Validate.isNullToDefault(map.get("COURSE_NAME"),"")%></td>
				<td style="width: 120px;"><%=Validate.isNullToDefault(map.get("NJMC"),"")%></td>
				<td style="width: 120px;"><%=Validate.isNullToDefault(map.get("VERSIONNAME"),"")%></td>
			</tr>
			<%}%>
		</table>
	</div>
				...
				handler : function() {
					...
					if($checkeds.length<1){
						...
						if(confirm("确认删除?")){
							...
							$("[name='P_NJBBXKRELATION_KCID_S_WD']").val(ids.join("','"));
							$("#pageForm").submit();
						}
					}
				}
