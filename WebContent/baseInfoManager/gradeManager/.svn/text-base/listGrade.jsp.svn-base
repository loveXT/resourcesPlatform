...
<body>
	...
	<form id="pageForm" class="sm_search" action="" method="post">
		...
		<input type="hidden" name="P_GRADE_ID_S_WD" value="">
	</form>
	<div class="sm_list">
		<!--列表  标题-->
		<table>
		...
		<table>
			<%
				...
				PageInfo pageInfo = gradeManager.findPageInfoByGradeManager(nowPage, pageSize, "_", request);
			...
			<tr>
				...
				<td style="width: 100px;"><%=Validate.isNullToDefault(map.get("NJMC"),"")%></td>
			</tr>
			<%}%>
		</table>
	</div>
...
<script type="text/javascript">
		$(function() {
			...
			$(".tool").toolBar([
			{
				...
				position : "-40px -160px",
				handler : function() {
					var src = "addGrade.jsp";
					jumpWindow("新增年级","550px","200px",src);					
				}
			},{
				text : "修改",
				width : "60",
				...
				handler : function() {
					...
					if($checked.length!=1){
						...
						var id = $checked.val();
						var src = "updateGrade.jsp";
						src +="?id="+id;
						jumpWindow("修改年级","550px","200px",src);		
					}
				}
			},{
				text : "刪除",
				...
				handler : function() {
					...
					if($checkeds.length<1){
						...
						if(confirm("确认删除?")){
							...
							$("[name='P_GRADE_ID_S_WD']").val(ids.join("','"));
							$("#pageForm").submit();
						}
					}
				}
