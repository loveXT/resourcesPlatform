...
<body>
	...
	<form id="pageForm" class="sm_search" action="" method="post">
		...
		<input type="hidden" name="P_MAINTYPEMANAGER_ID_S_WD" value="">
	</form>
	<div class="sm_list">
		<!--列表  标题-->
		<table>
		...
		<table>
			<%
				...
				PageInfo pageInfo = mainTypeManager.findPageInfoByMainTypeManager(nowPage, pageSize, "_", request);
			...
			<tr>
				...
				<td style="width: 120px;"><%=Validate.isNullToDefault(map.get("TYPENAME"),"")%></td>
				<td style="width: 300px;"><%=Validate.isNullToDefault(map.get("REMARK"),"")%></td>
			</tr>
			<%}%>
		</table>
	</div>
...
<script type="text/javascript">
		$(function() {
			...
			$(".tool").toolBar([
			{
				...
				position : "-40px -160px",
				handler : function() {
					var src = "addMainType.jsp";
					jumpWindow("新增版本","550px","260px",src);					
				}
			},{
				text : "修改",
				width : "60",
				...
				handler : function() {
					...
					if($checked.length!=1){
						...
						var id = $checked.val();
						var src = "updateMainType.jsp";
						src +="?id="+id;
						jumpWindow("修改版本","550px","260px",src);			
					}
				}
			},{
				text : "刪除",
				...
				handler : function() {
					...
					if($checkeds.length<1){
						...
						if(confirm("确认删除?")){
							...
							$("[name='P_MAINTYPEMANAGER_ID_S_WD']").val(ids.join("','"));
							$("#pageForm").submit();
						}
					}
				}
