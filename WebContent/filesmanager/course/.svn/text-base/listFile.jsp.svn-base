...
<div class="sm_list">	
	...
	<table>
		<%
			...
			pageInfo = filesManager.findByPrivateAllAndVerified(nowPage, pageSize, "_", request,state);
...
// 					var verify = $checkeds.siblings(".checkedddd").val();
		...
		if("sub" != verifys && "ver" != verifys && "dver" != verifys && "" != verifys && !deletePower){
			//如果审核已通过,并且没有删除权限，则不显示前面的按钮
			$(".tool").toolBar([]);
		}else{
			//若选择未提交和打回，则显示前面三个按钮
			//按钮的顺序
