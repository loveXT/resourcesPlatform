...
<%@page import="groovy.lang.Category"%>
...
<%
	...
	if (Validate.noNull(operateType)) {
			...
			if (extendName.equals(".rm") || extendName.equals(".rmvb")
				...
				response.sendRedirect(request.getContextPath()+sp+"uploadMovie.do?POINT_ID="+POINT_ID+"&CHAPTER_ID="+CHAPTER_ID+"&COURSE_ID="+COURSE_ID+"&MAJOR_ID="+Major_id+"&fileName="+fileName+"&h="+h+"&m="+m+"&s="+s+"&extendName="+extendName);
...
<%
	...
	pageInfo = folderCtr.findPageInfoByFOLDER1(nowPage, pageSize, "_", request, Major_id, CHAPTER_ID, COURSE_ID, POINT_ID);
...
<center class="sm_edit_center">
	<form class="sm_edit_from" method="post" action="" style="margin-top: 1%;height: 340px;">
		...
		<input type="hidden" name="P=FILES=BROWSETIME=S=C" value="0">
		<input type="hidden" name="P=FILES=DOWNLOADTIME=S=C" value="0">
		<input type="hidden" name="h" value="">
		<input type="hidden" name="m" value="">
		<input type="hidden" name="s" value="">
		<input type="hidden" name="P=FILES=FLAG=S=C" value="00"/>
		...
		<div class="sm_edit_td_name">输入原文件名</div>
			...
			<input type="hidden" name="P=FILES=MAJOR_ID=S=C" value="<%=map.get("MAJOR_ID") %>">
			<div class="sm_edit_td_value"><input type="text"  readonly="readonly" value="<%=map.get("MAJOR_NAME")== null?"公共基础课":map.get("MAJOR_NAME")%>"></div>
		</div>
		...
		<div class="sm_edit_tr">
			<div class="sm_edit_td_name">文件类别</div>
			<div class="sm_edit_td_value">
...
<!-- 			<input type="text" class="null" name="P_FILES_REMARK_S_C" /> -->
			<textarea style="height:50px;width:420px;" class="maxLength:40num null" name="P=FILES=REMARK=S=C"><%=Validate.isNullToDefault(request.getParameter("P_FILES_REMARK_S_C"), "")  %></textarea> 
			</div>
		</div>
...
<script type="text/javascript">
		...
		$(function() {
			...
			if("<%=request.getParameter("view")%>"=="true"){
				parent.location.href = "listFile.jsp?POINT_ID=<%=POINT_ID%>&CHAPTER_ID=<%=CHAPTER_ID%>&COURSE_ID=<%=COURSE_ID%>&MAJOR_ID=<%=Major_id%>";
			}
			$(".tool").toolBar([ {
				text : "保存",
				width : 50,
