			...
			.extend({
				jumpWinow : function(options) {
					...
					if(parent == ""){
						...
						if(options.windowsId != ""){
							var windows = $(this).find("#"+options.windowsId);
							var windowTitle = windows.find(".jumpWindow_title")[0];
							if( windows != undefined && windowTitle != undefined ){
								top += windows.offset().top + windowTitle.offsetHeight;
								left += windows.offset().left;
							}
						}
					}
					...
					// 创建关闭按钮
					$close = $("<div></div>").click(function(event) {
					...
					// 创建iframe
					src += "";
					$iframe = $("<iframe id='"+options.iframeId+"' frameborder='0'name='"+options.iframeId+"' src=''></iframe>").css({
						'width' : $main.outerWidth() - 12,
						"height" : height - 4
					}).addClass("jumpWindow_iframe").attr("src", addPosToSrc(src, top, left, id));
					if(parent == ""){
						// 添加模糊态
						$model.appendTo($this);
					}
					...
					$close.mousemove(function(event){
						event.stopPropagation();						
					});
...
function addPosToSrc(src, top, left, id){
	if(src.indexOf("?") < 0){
		src += "?1=1";
	}
	src += "&top=" + top + "&left=" + left + "&windowsId=" + id;
	return src;
}
...
//select型弹窗应用
function selectWindow(title,src,parent, windowsId){
	$("body").jumpWinow({
		title : title,
		src : src,
		width : "250",
		...
		parent : parent,
		windowsId : windowsId,
		iframeId : "selectWindow_iframe"
	});
}
//去除弹窗
function removeJumpWindow(){
	$(".jumpWindow_main").length;
...
function selectWindowToValue(parentId,$infoTree,only,bottom, windowsId, isToParent){
	var bool = true;
	var $parent = $(window.parent.document);
	if(isToParent){ 
		try{
			$parent = $(window.parent.jumpWindow_iframe.document);
		}catch(e){
	...
	if(bool){
		$(window.document).find("#"+windowsId).remove();
		$(window.parent.document).find("#"+windowsId).remove();
	}
}
