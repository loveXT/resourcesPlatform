...
public class UploadMovieServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		long screenshotTime = (Long.parseLong(h) * 60 + Long.parseLong(m) * 60 + Long.parseLong(s));
			extension=FilenameUtils.getExtension(fileName);
				...
				response.sendRedirect(contextPath+previewPage+"?view=true&FOLDERID="+FOLDERID+"&CHAPTER_ID="+CHAPTER_ID+"&COURSE_ID="+COURSE_ID+"&Major_id="+MAJOR_ID);
	}
	/**
	 * 用于判断文件是否是所需的视频格式
	 * @param extension
	...
	private static final String previewPage ="/filesmanager/fileManager/addFile.jsp"; 
}
