...
public class JpegTool {
	...
	public void doFinal(String pic_big_pathfilename,
		 ...
		 * (!pic_big_pathfilename.toLowerCase().endsWith("jpeg"))){ throw new
		 * JpegToolException(" 只能处理 JPG/JPEG 文件！ "); }
		 * if((!pic_small_pathfilename.toLowerCase().endsWith("jpg")) &&
		 * !pic_small_pathfilename.toLowerCase().endsWith("jpeg")){ throw new
		 * JpegToolException(" 只能处理 JPG/JPEG 文件！ "); }
		 */
		...
		// 输出小图像
		try {
			ImageIO.write(bsmall,
					FileUtil.getFileExtension(pic_small_pathfilename), fo);
